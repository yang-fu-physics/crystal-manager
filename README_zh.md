...
- **基本信息** — 样品编号、目标产物、成功/失败/待定、生长流程、结果、备注
- **元素质量计算** — 输入元素摩尔比 + 某参考元素质量，自动计算其余元素称量质量
- **实物照片** — 多张照片上传（支持拍照）、自动生成缩略图（按需极速加载）、点击放大预览原图
- **EDX 能谱分析** — 上传 EDX 谱图自动生成缩略图 → 调用 GPT Vision API 自动识别元素成分
